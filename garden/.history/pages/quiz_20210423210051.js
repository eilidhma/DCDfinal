...
export default function Quiz() {
  ...
  const initialClimate = localStorage.getItem('climate')
  const [climate, setClimate] = useState(initialClimate)
  const initialMaintenance = localStorage.getItem('maintenance')
  const [maintenance, setMaintenance] = useState(initialClimate)
  const initialGrowing = localStorage.getItem('growing')
  ...
  const HandleChange = () =>{
    ...
    if(question === questions.fourth.Q){
      ...
      // }
      useEffect(localStorage.setItem('type', type)
      )
      setHideState(false);
      router.push('/loading')
    }
