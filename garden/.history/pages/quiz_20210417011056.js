...
export default function Quiz() {
  ...
  const HandleChange = () =>{
    ...
    if(question === questions.first.Q){
      ...
      if(buttonstate === 1){
        reactLocalStorage.setObject('climate','temperate')
      }
      if(buttonstate === 2){
        reactLocalStorage.setObject('climate','tropical')
      }
      if(buttonstate === 3){
        reactLocalStorage.setObject('climate','arid')
      }
    ...
    if(question === questions.second.Q){
      ...
      if(buttonstate === 1){
        reactLocalStorage.setObject('maintenc'maintenance','low')
      }
      if(buttonstate === 2){
        reactLocalStorage.setObject('maintenance','medium')
      }
      if(buttonstate === 3){
        reactLocalStorage.setObject('maintenance','high')
      }
    }
    if(question === questions.third.Q){
      ...
      if(buttonstate === 1){
        reactLocalStorage.set('Good in','pots/planters')
      }
      if(buttonstate === 2){
        reactLocalStorage.set('Good in','ground')
      }
    ...
    if(question === questions.fourth.Q){
      ...
      if(buttonstate === 1){
        reactLocalStorage.set('Category','Vegetables/Fruits/Herbs')
      }
      if(buttonstate === 2){
        reactLocalStorage.set('Category','Flowers/Plants')
      }
